  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-[12px] z-20 flex">
        <div className="h-full w-[33%]" style={{ background: 'linear-gradient(270deg, #00FFA2 0%, #467664 99.09%)' }}></div>
      ...
      {/* --- 3. KOTAK PAKET TERPILIH --- */}
      <div 
        className="flex items-center gap-6 px-6 mb-10 shadow-lg transition-transform duration-500 hover:scale-105"
        style={{ width: '644px', height: '110px', background: '#2E4F4D', border: '1.5px solid #54868A', borderRadius: '18px', boxSizing: 'border-box' }}
      >
        <div className="flex items-center justify-center shadow-inner" style={{ width: '78px', height: '76px', background: '#B3D2D1', borderRadius: '9px' }}>
      ...
      {/* --- 4. QRIS --- */}
      <Link href={`/qris/${paketDipilih}`} className="group flex items-center justify-between px-6 mb-4 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_10px_20px_rgba(0,0,0,0.3)] hover:border-[#6AC5C3] no-underline" style={{ width: '644px', height: '108px', background: '#2E4F4D', border: '1.5px solid #54868A', borderRadius: '18px', boxSizing: 'border-box' }}>
        <div className="flex items-center gap-6">
          <div className="flex items-center justify-center transition-transform group-hover:scale-110" style={{ width: '78px', height: '76px', background: '#B3D2D1', borderRadius: '9px' }}>
            <img src="/qris.png" alt="QRIS" style={{ width: '50px', height: '50px', objectFit: 'contain' }} />
          </div>
          <div className="flex flex-col">
            <h2 className="transition-colors group-hover:text-[#6FFFB4]" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '32px', letterSpacing: '-0.05em', color: '#FFF', lineHeight: '1.1' }}>Scan QR Code</h2>
            <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: '16px', letterSpacing: '-0.05em', color: '#D1D1D1' }}>GoPay - OVO - Dana - DLL</p>
          </div>
        </div>
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="transition-all duration-300 opacity-60 group-hover:opacity-100 group-hover:translate-x-3 group-hover:stroke-[#6FFFB4]"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
      </Link>

      {/* --- 5. VOUCHER --- */}
      <Link href="/kupon" className="group flex items-center justify-between px-6 mb-12 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_10px_20px_rgba(0,0,0,0.3)] hover:border-[#6AC5C3] no-underline" style={{ width: '644px', height: '108px', background: '#2E4F4D', border: '1.5px solid #54868A', borderRadius: '18px', boxSizing: 'border-box' }}>
        <div className="flex items-center gap-6">
          <div className="flex items-center justify-center transition-transform group-hover:scale-110" style={{ width: '78px', height: '76px', background: '#B3D2D1', borderRadius: '9px' }}>
            <img src="/voucher.png" alt="Voucher" style={{ width: '50px', height: '50px', objectFit: 'contain' }} />
          </div>
          <div className="flex flex-col">
            <h2 className="transition-colors group-hover:text-[#6FFFB4]" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '32px', letterSpacing: '-0.05em', color: '#FFF', lineHeight: '1.1' }}>Kode Voucher</h2>
      ...
      {/* --- 6. TOMBOL GANTI PAKET --- */}
      <Link 
        href="/pilih-paket" 
        className="flex items-center justify-center gap-3 transition-transform hover:scale-105 active:scale-95 no-underline"
        style={{ width: '286px', height: '48px', background: '#224C42', border: '3px solid #318570', borderRadius: '30px' }}
      >
        ...
        <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: '20px', letterSpacing: '-0.05em', color: '#132E27' }}>Ganti Paket</span>
      </Link>
    </main>
  );
}