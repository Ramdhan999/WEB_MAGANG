...
import Script from "next/script"; // <-- WAJIB IMPORT INI BRO
...
function QrisContent() {
  ...
  const hasTriggeredRef = useRef(false); // Biar popup ga muncul double
  ...
  useEffect(() => {
    ...
    const generatePayment = async () => {
      ...
      try {
        console.log("Nembak backend buat dapet token Midtrans...");
        const res = await fetch("http://localhost:8080/api/payment", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ paket: paketDipilih }),
        });
        ...
        if (data.token) {
          ...
          (window as any).snap.pay(data.token, {
            onSuccess: function () {
              console.log("LUNAS BRO!");
              router.push("/success"); // Langsung pindah ke halaman sukses!
            },
            onPending: function () {
              console.log("Menunggu pembayaran...");
            },
            onError: function () {
              router.push(`/gagal?reason=error&paket=${paketDipilih}`);
            },
            onClose: function () {
    ...
    generatePayment();
  }, [isSnapLoaded, paketDipilih, router]);
  ...
  return (
    <>
      {/* EMBED SCRIPT MIDTRANS SECARA NATIVE ALA NEXT.JS */}
      <Script
