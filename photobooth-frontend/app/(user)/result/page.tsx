...
function ResultContent() {
  ...
  const getFilterCSS = () => {
    ...
    switch (activeFilter) {
      case "NOIR": return `grayscale(${100 * val}%) contrast(${100 + (20 * val)}%)`;
      case "VINTAGE": return `sepia(${60 * val}%) contrast(${100 - (10 * val)}%)`;
      case "COOL": return `hue-rotate(${30 * val}deg) saturate(${100 + (20 * val)}%)`;
      case "WARM": return `sepia(${30 * val}%) saturate(${100 + (40 * val)}%)`;
      case "VIVID": return `saturate(${100 + (80 * val)}%)`;
      case "DRAMA": return `contrast(${100 + (50 * val)}%) saturate(${100 - (20 * val)}%)`;
      case "SOFT": return `brightness(${100 + (10 * val)}%) contrast(${100 - (15 * val)}%) blur(${1 * val}px)`;
      case "FILM": return `sepia(${20 * val}%) contrast(${100 + (10 * val)}%) brightness(${100 - (5 * val)}%)`;
      default: return "none";
    }
  };
  ...
  // --- LOGIKA MANGGIL POPUP MIDTRANS KHUSUS CETAK ---
  const handlePaymentCetak = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8080/api/payment", {
        ...
        body: JSON.stringify({ paket: `cetak_${extraCetak}` }),
      });
      ...
      if (data.token) {
        (window as any).snap.pay(data.token, {
          onSuccess: function () {
            console.log("Pembayaran cetak tambahan LUNAS!");
            setCetakStep('preview'); 
          },
          onPending: function () {
          ...
          onError: function () {
            ...
            setCetakStep('options'); // Kalau gagal, balikin ke opsi cetak
          },
          onClose: function () {
            console.log("Popup ditutup");
            setCetakStep('options'); // Kalau popup diclose tanpa bayar, balikin ke opsi cetak
          },
        });
      }
    } catch (error) {
  ...
  // AUTOTRIGGER MIDTRANS SAAT MASUK STEP QRIS
  useEffect(() => {
    if (cetakStep === 'qris' && extraCetak > 0) {
      handlePaymentCetak();
    }
  }, [cetakStep]);
          ...
          <div className="w-full max-w-[1800px] flex items-center justify-between px-8">
              ...
              <button onClick={handleNewSession} className="w-[200px] h-[60px] bg-[#CDCDCD] rounded-[13px] flex items-center justify-center gap-3 hover:bg-gray-300 shadow-md transition-all active:scale-95 group">
                <div className="w-6 h-6 flex items-center justify-center opacity-70 group-hover:-rotate-180 transition-transform duration-500">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#545454" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {cetakStep === 'qris' && (
                    <div className="flex flex-col items-center w-full p-6">
                        <div className="w-[64px] h-[64px] bg-[#FFF6E5] border border-[#D29E38] rounded-[20px] flex items-center justify-center mb-4 shadow-sm">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#D29E38" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <div className="flex items-center gap-2 mb-6 bg-[#F4F9F8] px-4 py-2 rounded-full border border-[#54868A]/30">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#545454" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <circle cx="12" cy="12" r="10" />
                            <polyline points="12 6 12 12 16 14" />
                          </svg>
                          <span className="font-inter font-medium text-[15px] text-[#545454]">Menunggu Pembayaran</span>
                        </div>

                        <div className="w-full flex justify-center">
                          <button onClick={() => setCetakStep('options')} className="w-full max-w-[200px] h-[53px] bg-white border border-[#54868A] rounded-[16px] font-inter font-bold italic text-[18px] text-[#545454] active:scale-95 hover:bg-[#F9F9F9] transition-colors shadow-sm pt-0.5">Batal</button>
                        </div>
                    </div>
                  )}
