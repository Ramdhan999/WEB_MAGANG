...
const IDLE_TIMEOUT_MS = 300_000; // 5 menit idle → auto kembali ke home
...
// ===== SEEDED SHUFFLE (konsisten berdasarkan seed string) =====
...
function ResultContent() {
  ...
  // ===== MODAL STATE =====
  // step 'options' = pilih qty + bayar
  // step 'printing' = lagi proses cetak (progress)
  ...
  // ===== REF buat capture FramePreview ke gambar =====
  const printRef = useRef<HTMLDivElement>(null);

  // =====================================================================
  // 💤 IDLE TIMEOUT — auto kembali ke home kalo gak ada aktivitas user
  // =====================================================================
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      idleTimer = setTimeout(() => {
        // Skip kalo lagi proses penting (print/payment/zip) — jangan ganggu user
        if (printStatus === 'rendering' || printStatus === 'printing' || isPaying || isZipping) {
          resetTimer();
          return;
        }
        // Aktivitas habis → reset sesi + ke home
    ...
    resetTimer(); // start counter pas mount

    return () => {
      clearTimeout(idleTimer);
      events.forEach(e => window.removeEventListener(e, resetTimer));
    };
  }, [router, printStatus, isPaying, isZipping]);

  // =====================================================================
  // POLLING Midtrans Snap loaded — FIX: kadang onLoad gak fire kalau cached
  // =====================================================================
  useEffect(() => {
  ...
  // Wait helper: poll sampe snap siap (max timeoutMs)
  ...
  // =====================================================================
  // 🖨️ RENDER FRAME KE BASE64 (pake html2canvas)
  // =====================================================================
  const renderFrameImage = async (): Promise<string | null> => {
    if (!printRef.current) return null;
    try {
      // Dynamic import biar gak ngeberatin initial load
      const html2canvas = (await import("html2canvas")).default;
      const canvas = await html2canvas(printRef.current, {
        scale: 5, // 5x upscale buat resolusi cetak (240*5=1200px wide)
  ...
  // =====================================================================
  // 🖨️ EXECUTE PRINT — kirim image base64 + qty ke backend
  // =====================================================================
  ...
  // =====================================================================
  // 🖨️ MAIN PRINT FLOW
  // - extraCetak = 0 → langsung print 1 lembar
  // - extraCetak > 0 → Midtrans → on success → print (1+extra) lembar
  // =====================================================================
  const handleCetak = async () => {
    const totalQty = 1 + extraCetak;

    // === KASUS 1: Ada cetakan tambahan → bayar dulu via Midtrans ===
    if (extraCetak > 0) {
      // Tunggu Midtrans Snap siap (kalau onLoad belum fire)
      ...
      try {
        ...
        (window as any).snap.pay(data.token, {
          onSuccess: async function () {
            ...
            // Pindah ke step printing, langsung trigger
    ...
    // === KASUS 2: Gak ada tambahan → langsung print 1 lembar ===
    await markPrintDone(0);
    setCetakStep('printing');
    await executePrint(1);
  };

  // ===== DIGITAL: tandai output_type=Digital pas buka modal =====
  ...
  // Tutup modal cetak — reset semua state
  const closeCetakModal = () => {
    if (isPaying || printStatus === 'rendering' || printStatus === 'printing') {
      return; // jangan bisa di-close pas lagi process
    }
    setActiveModal(null);
    setExtraCetak(0);
  ...
  // Dipake 2x: sekali di main view (dengan ref buat capture), sekali di modal preview.
  // ===== FRAME PREVIEW COMPONENT (clean, no white wrapper) =====
  const FramePreview = ({ className = "", innerRef }: { className?: string; innerRef?: React.Ref<HTMLDivElement> }) => (
    <div ref={innerRef} className={`w-[300px] h-[450px] relative overflow-hidden ${className}`} style={{ backgroundColor: '#545151' }}>
      <div className="absolute inset-0 z-10 transition-all duration-300" style={{ filter: getFilterCSS(), opacity: getFilterOpacity(), ...overlayStyle }}>
        {photoSlots.map((slot) => (
          <div key={slot.id} className="w-full h-full bg-[#979797] overflow-hidden flex items-center justify-center">
            {slot.photo ? <img src={slot.photo} crossOrigin="anonymous" className="w-full h-full object-cover" alt={`Slot ${slot.id}`} /> : <span className="text-[#264E45] font-hind font-bold text-[36px] opacity-30">{slot.id}</span>}
          </div>
        ))}
      </div>
      {framePath && <img src={framePath} crossOrigin="anonymous" className="absolute inset-0 w-full h-full object-fill z-20 pointer-events-none" alt="Frame" />}
  ...
  return (
    <>
      ...
      <main className="relative flex min-h-screen flex-col items-center pt-4 pb-[115px] overflow-x-hidden select-none" style={{ backgroundColor: '#E3D5D5' }}>
        ...
        <div className="w-full max-w-[1800px] flex flex-col xl:flex-row gap-8 items-stretch justify-center px-8 z-10">
          ...
          <div className="flex-1 max-w-[790px] min-h-[740px] rounded-[23px] p-8 flex flex-col items-center shadow-2xl relative overflow-hidden border border-[#54868A]/30"
            ...
            <div className="flex-1 w-full flex items-center justify-center relative pb-6 scale-100 xl:scale-105 origin-center">
              {/* INI YANG DI-CAPTURE buat print — passing ref */}
          ...
          <div className="flex-1 max-w-[970px] flex flex-col gap-6">
            ...
            {/* TOMBOL CETAK & DIGITAL */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full shrink-0">
              <button onClick={() => { setActiveModal('cetak'); setCetakStep('options'); setExtraCetak(0); setPrintStatus('idle'); setPrintErrorMsg(""); }} className="relative h-[95px] w-full bg-white border-[1.5px] border-[#54868A] rounded-[23px] flex items-center p-3 gap-4 shadow-sm hover:scale-[1.02] active:scale-95 transition-all text-left group">
              ...
              <button onClick={handleOpenDigital} className="relative h-[95px] w-full bg-white border-[1.5px] border-[#54868A] rounded-[23px] flex items-center p-3 gap-4 shadow-sm hover:scale-[1.02] active:scale-95 transition-all text-left group">
                ...
                <div className="flex-1 flex flex-col justify-center pr-8 pt-1">
                  ...
                  <p className="font-hind font-semibold text-[17px] text-[#3E8C7B] tracking-[-0.05em] leading-tight">WhatsApp &amp; QR galeri</p>
                </div>
                <span className="absolute right-6 text-[#54868A] opacity-50 text-3xl group-hover:translate-x-2 transition-transform">→</span>
              </button>
            </div>
        ...
        {/* FOOTER — cuma "Sesi Baru" doang, di-center */}
                  ...
                  <div className="w-full bg-[#FFF6E5] border border-[#F2E0C4] rounded-[20px] p-5 flex flex-col items-center">
                    <div className="flex items-center gap-3 w-full mb-4">
                      ...
                      <div className="flex flex-col flex-1">
                        ...
                        <span className="font-hind font-semibold text-[13px] text-[#D29E38] leading-tight max-w-[280px]">Aktif setelah booth online (hosting)</span>
                      </div>
                    </div>
                    <div className="w-[160px] h-[160px] bg-white rounded-xl shadow-md border border-[#54868A] p-3 flex items-center justify-center mb-3 opacity-60">
                      <img src="/qris1.png" className="w-full h-full object-contain" alt="QR galeri" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                    </div>
